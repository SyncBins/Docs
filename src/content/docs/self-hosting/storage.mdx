...
description: Local disk, AWS S3, Cloudflare R2, or Azure Blob — how SyncBins stores encrypted blobs and what to put in .env.
sidebar:
  order: 2
  badge:
    text: New
...
**Default for self-hosters: local disk.** No cloud account, no extra env vars beyond the [quick start](/self-hosting/quickstart/). Switch to S3, R2, or Azure only if you want blobs off the VPS or need more durability than a single disk.
...
<Tabs syncKey="storage-backend">
  ...
  <TabItem label="AWS S3">
    ...
    Create a bucket in the AWS console (or via CLI), generate an IAM user with `s3:PutObject`, `s3:GetObject`, `s3:DeleteObject`, and `s3:HeadObject` permissions scoped to the bucket, then paste the credentials into `.env`.
...
<Aside type="tip" title="S3-compatible services">
The `s3` backend works with any S3-compatible service. Set `S3_ENDPOINT` to point at **MinIO**, **DigitalOcean Spaces**, **Backblaze B2**, or similar. The `r2` backend remains a separate option with its own R2-specific endpoint logic.
</Aside>
